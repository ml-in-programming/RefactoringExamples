...
public class Validator {
	...
	private boolean validate(SpoutcraftData build) throws RestfulAPIException {
		File minecraftJar = new File(UpdateThread.getBinDir(Launcher.getGameUpdater()), "minecraft.jar");
		...
		File spoutcraft = new File(UpdateThread.getBinDir(Launcher.getGameUpdater()), "spoutcraft.jar");
		...
		File libDir = new File(UpdateThread.getBinDir(Launcher.getGameUpdater()), "lib");
		...
		for (Library lib : allLibraries) {
			...
			if ((lib.name().contains("lwjgl") || lib.name().contains("jinput"))&& !lib.getVersion().contains("natives")) {
				libraryFile = new File(UpdateThread.getBinDir(Launcher.getGameUpdater()), lib.getArtifactId() + ".jar");
