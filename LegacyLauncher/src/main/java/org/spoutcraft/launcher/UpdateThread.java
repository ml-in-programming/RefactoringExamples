...
public class UpdateThread extends Thread {
	...
	final DownloadListener listener = new DownloadListenerWrapper();
	...
	// TODO move to StartupParameters
    public static boolean isIgnoreMD5(StartupParameters startupParameters) {
        return startupParameters.ignoreMD5;
    }

    // TODO move to SpoutcraftDirectories
	public static File getBinDir(SpoutcraftDirectories spoutcraftDirectories) {
		return spoutcraftDirectories.binDir;
	}

	// TODO move to SpoutcraftDirectories
	public static File getBinCacheDir(SpoutcraftDirectories spoutcraftDirectories) {
		return spoutcraftDirectories.binCacheDir;
	}

	// TODO move to SpoutcraftDirectories
	public static File getSpoutcraftDir(SpoutcraftDirectories spoutcraftDirectories) {
		return spoutcraftDirectories.spoutcraftDir;
	}

	// TODO move to SpoutcraftDirectories
	public static File getUpdateDir(SpoutcraftDirectories spoutcraftDirectories) {
		return spoutcraftDirectories.updateDir;
	}
	...
	private void runTasks() throws IOException {
		while (!valid.get()) {
			...
			if (!(isIgnoreMD5(params) || Settings.isIgnoreMD5())) {
				validate.run(build);
	...
	private void updateAssets(Map<String, Object> assets, File directory) {
		...
		for (Entry<String, Object> entry : assets.entrySet()) {
			...
			if (value instanceof Map<?, ?>) {
				...
				if (asset.exists() && !(isIgnoreMD5(params) || Settings.isIgnoreMD5())) {
				...
				} else if (asset.exists() && (isIgnoreMD5(params) || Settings.isIgnoreMD5())) {
					needDownload = false;
				}
	...
	private void updateFiles() {
		...
		if (oldConfig.exists() && oldConfig.isDirectory()) {
			moveDirectory(oldConfig, getSpoutcraftDir(dirs));
	...
	public boolean isSpoutcraftUpdateAvailable(SpoutcraftData build) throws RestfulAPIException {
		...
		if (!getSpoutcraftDir(Launcher.getGameUpdater()).exists()) {
			return true;
		}
		...
		progress += 100F;
		File spoutcraft = new File(getBinDir(Launcher.getGameUpdater()), "spoutcraft.jar");
		...
		stateChanged("Checking for Spoutcraft update...", progress / steps);
		progress += 100F;
		File libDir = new File(getBinDir(Launcher.getGameUpdater()), "lib");
		libDir.mkdir();
	...
	public boolean isMinecraftUpdateAvailable(SpoutcraftData build) {
		int steps = 7;
		if (!getBinDir(Launcher.getGameUpdater()).exists()) {
			return true;
		}
		stateChanged("Checking for Minecraft update...", 100F / steps);
		File nativesDir = new File(getBinDir(Launcher.getGameUpdater()), "natives");
		...
		File minecraft = new File(getBinDir(Launcher.getGameUpdater()), "minecraft.jar");
		if (!minecraft.exists()) {
			return true;
		}
		stateChanged("Checking for Minecraft update...", 300F / steps);
		File lib = new File(getBinDir(Launcher.getGameUpdater()), "jinput.jar");
		if (!lib.exists()) {
			return true;
		}
		stateChanged("Checking for Minecraft update...", 400F / steps);
		lib = new File(getBinDir(Launcher.getGameUpdater()), "lwjgl.jar");
		if (!lib.exists()) {
			return true;
		}
		stateChanged("Checking for Minecraft update...", 500F / steps);
		lib = new File(getBinDir(Launcher.getGameUpdater()), "lwjgl_util.jar");
		if (!lib.exists()) {
			return true;
		}
	...
	public void updateMinecraft(SpoutcraftData build) throws IOException {
		getBinDir(Launcher.getGameUpdater()).mkdir();
		getBinCacheDir(Launcher.getGameUpdater()).mkdir();
		if (getUpdateDir(Launcher.getGameUpdater()).exists()) {
			FileUtils.deleteDirectory(getUpdateDir(Launcher.getGameUpdater()));
		}
		getUpdateDir(Launcher.getGameUpdater()).mkdir();
		...
		File mcCache = new File(getBinCacheDir(Launcher.getGameUpdater()), "minecraft_" + minecraft.getVersion() + ".jar");
		...
		Utils.copy(mcCache, new File(getBinDir(Launcher.getGameUpdater()), "minecraft.jar"));
	...
	public void updateSpoutcraft(SpoutcraftData build) throws IOException {
		...
		getUpdateDir(Launcher.getGameUpdater()).mkdirs();
		getBinCacheDir(Launcher.getGameUpdater()).mkdirs();
		getSpoutcraftDir(Launcher.getGameUpdater()).mkdirs();
		File cacheDir = new File(getBinDir(Launcher.getGameUpdater()), "cache");
		cacheDir.mkdir();

		// Process spoutcraft.jar
		logger.info("Spoutcraft Build: " + build.getBuild());
		File mcCache = new File(getBinCacheDir(Launcher.getGameUpdater()), "minecraft_" + build.getMinecraft().getVersion() + ".jar");
		File updateMC = new File(getUpdateDir(Launcher.getGameUpdater()).getPath() + File.separator + "minecraft.jar");
		if (mcCache.exists()) {
		...
		if (!spoutcraft.exists()) {
			Download download = DownloadUtils.downloadFile(url, getUpdateDir(Launcher.getGameUpdater()) + File.separator + "spoutcraft.jar", null, build.getMD5(), listener);
		...
		for (Library lib : allLibraries) {
			...
			if (lib.getArtifactId().contains("lwjgl") || lib.getArtifactId().contains("jinput")) {
				...
				if (lib.getVersion().contains("natives")) {
					File nativesFolder = new File(getBinDir(Launcher.getGameUpdater()), "natives");
					...
					File nativesJar = new File(getUpdateDir(Launcher.getGameUpdater()), "natives.jar");
					if (!nativesJar.exists() || !lib.valid(MD5Utils.getMD5(nativesJar))) {
						...
						File tempNatives = new File(getUpdateDir(Launcher.getGameUpdater()), "natives");
				...
				libraryFile = new File(getBinDir(Launcher.getGameUpdater()), lib.getArtifactId() + ".jar");
			}
	...
	public void cleanupBinFoldersFor(SpoutcraftData build) {
		try {
			...
			for (File file : getBinDir(Launcher.getGameUpdater()).listFiles()) {
				if (!file.isFile()) {
					continue;
				}
