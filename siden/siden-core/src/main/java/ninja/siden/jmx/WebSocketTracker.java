...
public class WebSocketTracker implements WebSocketFactory, WebSocketMXBean {
	...
	// TODO move to RequestMeter
    public static RequestMetrics toMetrics(RequestMeter requestMeter) {
        return new RequestMetrics(new Date(requestMeter.startTime),
                requestMeter.totalRequestTime.sum(), requestMeter.maxRequestTime.get(),
                requestMeter.minRequestTime.get(), requestMeter.totalRequests.sum());
    }
